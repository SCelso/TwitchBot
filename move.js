...
const channel = process.env.TWITCH_CHAT_BOT_CHANNEL;
const account = process.env.TWITCH_CHAT_BOT_ACCOUNT;
const password = process.env.TWITCH_CHAT_BOT_PASSWORD;
    ...
    connection.on('message', function(ircMessage) {
        if (ircMessage.type === 'utf8') {
            ...
            messages.forEach(message => {
                ...
                if (parsedMessage) {
                    console.log(`Message received (${new Date().toISOString()}): ${parsedMessage.source.nick}: ${parsedMessage.parameters}`);                    // console.log(`\n${JSON.stringify(parsedMessage, null, 3)}`)
                    switch (parsedMessage.command.command) {
                        case '001':
                            // Successfully logged in, so join the channel.
                            connection.sendUTF(`JOIN ${channel}`); 
                            connection.sendUTF(`PRIVMSG #kingdaddyjr :HeyGuys <3 PartyTime`);
                            break; 
                    }
                }
            });
...
//functions to print chat into terminal
